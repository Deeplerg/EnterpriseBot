...
{
    public abstract class LocalizationCategoryBase<TModel, TId, TCreationParams>
                                       : ICategory<TModel, TId, TCreationParams> where TModel : class
                                                                                 where TCreationParams : class
    {
        protected const string categoryAreaName = "Localization";
        protected readonly IApiClient api;

        public LocalizationCategoryBase(IApiClient apiClient)
        {
            this.api = apiClient;
        }
